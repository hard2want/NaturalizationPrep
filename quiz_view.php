        ...
        <main id="qSet">
            <form action="." method="POST">                                                                                     
                ...
                <?php   $answerSet[0] = implode($allCorrectAnswers[$indexTracker[$i]]);                                         //  add the correct answer to the answer set
                        foreach($allIncorrectAnswers[$indexTracker[$i]] as $incorrect){                                         // add incorrect answers to the answer set
                            array_push($answerSet, $incorrect);}
                        if ($answerSet[0] == "All of the above"){                                                               // if "all of the above", sort so answer is last via krsort()
                            krsort($answerSet);                                                                                 
                        } else {asort($answerSet);}?><br>                                                                       <!-- else, sort answers alphabetically -->
                <?php   foreach  ($answerSet as $key => $answer) : ?>
                            <input type="radio" name="<?php echo $i; ?>" value="<?php echo $key; ?>"> <?php echo $answer; ?><br><!-- present each answer with a radio dial option -->
                <?php   endforeach;                                                                                             
